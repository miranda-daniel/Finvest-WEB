...
const columns = [
  columnHelper.accessor('instrument.symbol', {
    header: 'Symbol',
    cell: (info) => (
      <span className="text-sm font-semibold text-slate-100">{info.getValue()}</span>
    ),
  }),
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            {portfolio?.description && (
              <p className="mt-1 text-subtle">{portfolio.description}</p>
            )}
          </div>
        </div>
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id} className="border-b border-white/8">
                {headerGroup.headers.map((header) => (
                  <th
                    key={header.id}
                    className="px-5 py-3.5 text-left text-label"
                  >
